...
| `error: Microsoft Visual C++ 14.0 or greater is required` khi cài `pyradiomics` | Thiếu trình biên dịch C++ trên Windows. | Cài đặt **Visual Studio C++ Build Tools** hoặc dùng Python 3.10 để tải wheel nhị phân dựng sẵn. Hoặc chỉ cài `requirements.txt` nếu chỉ cần chạy Dashboard. |
